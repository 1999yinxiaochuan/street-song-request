...
function renderPendingOrders(orders) {
    ...
    c.innerHTML = orders.map(function(o) {
        ...
        return '<div class="order-card ' + sc + '"><div class="order-header"><span class="order-id">订单 #' + o.id + '</span><span class="order-time">' + formatTime(o.createdAt) + '</span></div><div class="order-body"><div class="order-songs">' + o.songs.map(function(s) { return '<span class="order-song-tag">《' + escapeHtml(s) + '》</span>'; }).join('') + '</div><div class="order-info"><span class="order-requester">点歌人：' + escapeHtml(o.requester) + '</span>' + (o.contact ? '<span class="order-contact">联系方式：' + escapeHtml(o.contact) + '</span>' : '') + '</div><div class="order-total"><span>总计：</span><span class="total-price">¥' + o.totalPrice + '</span></div></div><div class="order-actions"><span class="order-status ' + sc + '">' + st + '</span>' + act + '</div></div>';
    }).join('');
}
